...
if ($opt_a) {
	...
	print STDERR "The specified threshold is $threshold\n";
}
...
sub adjust_abundance {
	...
	## Return data if no genes in high abundance class
	if ($high_abundance_total == 0) {
		# Most likely with non-lactating or control data
		print "GeneID\tRawExpression\n";
		while (my ($gene, $raw_freq) = each %genes_to_frequency) {
